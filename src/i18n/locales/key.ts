export const TRANSLATION_KEY = {
  MODE_SELECT_TIP: "mode_select_tip",
  NAV_HOME: "nav.home",
  NAV_DATA: "nav.data",
  NAV_CONNECTION: "nav.connection",
  LOG: "common.log",
  NAV_SETTING: "nav.setting",
  NAV_ABOUT: "nav.about",
  CONNECTIVITY_TEST: "common.connectivity_test",
  COMMON_TEST_UDP: "common.test_upd",
  COMMON_EDIT: "common.edit",
  COMMON_DELETE: "common.delete",
  COMMON_COPY_URL: "common.copy_url",
  COMMON_QR_CODE: "common.qr_code",
  FORM_NAME: "form.name",
  MODE: "common.mode",
  HOST: "common.host",
  TLS: "common.tls",
  SKIP_CERT_VERIFY: "skip_cert_verify",
  DISABLE_RESUE: "disable_resue",
  ECH_OPTS: "ech_opts",
  ENABLE: "enable",
  CONFIG: "config",
  QUERY_SERVER_NAME: "query_server_name",
  VERSION: "version",
  SHADOW_TLS_OPTIONS: "shadow_tls_options",
  RESTLS_OPTIONS: "restls_options",
  VERSION_HINT: "version_hint",
  RESTLS_SCRIPT: "restls_script",
  JLS_OPTS: "jls_opts",
  CLIENT_METADATA: "client-metadata",
  PATH: "common.path",
  FORM_SERVER: "form.server",
  FORM_ENCRYPTION: "from.encryption",
  FORM_PORT: "form.port",
  FORM_PASSWORD: "form.password",
  FORM_SNI: "form.sni",
  FORM_CLIENT_FINGERPRINT: "form.client_fingerprint",
  FORM_FINGERPRINT: "form.fingerprint",
  FORM_CERTIFICATE: "form.certificate",
  FORM_PRIVATE_KEY: "form.private_key",
  FORM_IDLE_SESSION_CHECK_INTERVAL: "form.idle_session_check_interval",
  FORM_IDLE_SESSION_TIMEOUT: "form.idle_session_timeout",
  FORM_MIN_IDLE_SESSION: "form.min_idle_session",
  FORM_SKIP_CERT_VERIFY: "form.skip_cert_verify",
  FORM_ALPN: "form.alpn",
  FORM_PLUGIN: "form.plugin",
  FORM_OPTIONAL: "form.optional",
  FORM_CANCEL: "form.cancel",
  FORM_SAVE: "form.save",
  FORM_USERNAME: "form.username",
  FORM_RESET: "form.reset",
  FORM_UDP_OVER_TCP: "form.udp_over_tcp",
  FORM_UDP_OVER_TCP_VERSION: "form.udp_over_tcp_version",
  FORM_PLUGIN_OPTS: "form.plugin_opts",
  FORM_UDP: "form.udp",
  DELETE_PROXIES: "delete_proxies",
  COMMON_RUNTIME_DETAIL: "common.runtime_detail",
  COMMON_TUN_INTERFACE_NAME: "common.tun_interface_name",
  COMMON_DEFAULT_INTERFACE_NAME: "common.default_interface_name",
  DEFAULT_INTERFACE_V4_ADDR: "default_interface_v4_addr",
  SETTING_PRIMARY_DNS: "setting.primary_dns",
  SETTING_SECONDARY_DNS: "setting.secondary_dns",
  SWITCH_DISABLE_TIP: "switch.disable.tip",
  DEFAULT_INTERFACE: "default_interface",
  DEFAULT_INTERFACE_TOOLTIP: "default_interface_tooltip",
  PROXY_SERVER_MSG: "proxy_server_msg",
  SAVE_SUCCESS: "save_success",
  UPDATE_SUCCESS: "update_success",
  DOWNLOAD: "common.download",
  DATA: "common.data",
  UPLOAD: "common.upload",
  DESTINATION: "common.destination",
  PROCESS: "common.process",
  Domain: "common.domain",
  RULE: "common.rule",
  NETWORK: "common.network",
  TIME: "common.time",
  CLOSE_ALL: "close_all",
  CLOSE: "common.close",
  TYPE: "common.type",
  CONTENT: "common.content",
  ELEVATE_CORE: "elevate_core",
  ELEVATE_TIP_MACOS: "elevate_tip_macos",
  ELEVATE_TIP_WINDOWS: "elevate_tip_windows",
  DELETE_ALL_PROXIES_TIPS: "delete_all_proxies_tips",
  UNKNOWN_ERROR: "unknown_error",
  SHADOWSOCKS: "common.shadowsocks",
  ANYTLS: "common.anytls",
  SOCKS5: "common.socks5",
  HTTP: "common.http",
  PROXY_TEXT_IMPORT: "proxy_text_import",
  SUBSCRIPTION_URL_IMPORT: "subscription_url_import",
  SUBSCRIPTION: "subscription_url",
  PROXY_URI: "proxy_uri",
  CLASH_YAML: "clash_yaml",
  YAML: "yaml",
  SEE: "see",
  HOW_TO_CONFIG: "how_to_config",
  EDIT_YAML: "edit_yaml",
  ERROR: "common.error",
  WARNING: "common.warning",
  INFO: "common.info",
  DEBUG: "common.debug",
  DISCONNECTED_WARN_TIP: "disconnected_warn_tip",
  PROXY: "common.proxy",
  DIRECT: "common.direct",
  COPIED: "common.copied",
  REJECT: "common.reject",
  DELETED: "common.deleted",
  SEARCH: "common.search",
  HTTP_SERVER_SWITCH_LABEL: "http_server_switch_label",
  HTTP_SERVER_SWITCH_TOOLTIP: "http_server_switch_tooltip",
  MODE_SWITCH_LABEL: "mode_switch_label",
  OPEN_DIR: "open_dir",
  CONFIG_FILE: "config_file",
  CONFIG_FILE_TIP: "config_file_tip",
  MODE_SWITCH_TOOLTIP: "mode_switch_tooltip",
  TESTING_URL_LABEL: "testing_url_label",
  TESTING_URL_DESC: "testing_url_desc",
  PROXY_MODE_TYPE_LABEL: "proxy_mode_type_label",
  BOOST_DNS_LABEL: "boost_dns_label",
  BOOST_DNS_DESC: "boost_dns_desc",
  REMOTE_DNS_LABEL: "remote_dns_label",
  REMOTE_DNS_DESC: "remote_dns_desc",
  LOCAL_DNS_LABEL: "local_dns_label",
  LOCAL_DNS_DESC: "local_dns_desc",
  PROXY_MODE_TYPE_DESC: "proxy_mode_type_desc",
  HTTP_SERVER_PORT_LABEL: "http_server_port_label",
  HTTP_SERVER_PORT_DESC: "http_server_port_desc",
  UDP_FAILED_NOTIFICATION: "udp_failed_notification",
  UDP_OK_NOTIFICATION: "udp_ok_notification",
  MORE: "common.more",
  NEW_PROXY: "new_proxy",
  OPEN_LOG_DIR: "open_log_dir",
  HUB_PORT: "hub_port",
  HUB_ADDRESS: "hub_address",
  BOOST_DNS: "boost_dns",
  PROXY_SERVER: "proxy_server",
  SEARCH_LOG_TIP: "search_log_tip",
  NEW_VERSION_INFO: "new_version_info",
  CHECK_UPDATE: "check_update",
  CONFIRM: "common.confirm",
  GO: "common.go",
  OK: "common.ok",
  NO_UPDATE_INFO: "no_update_info",
  PROXY_ALL_RULE: "proxy_all",
  BYPASS_ALL_RULE: "bypass_all",
  BYPASS_CN_RULE: "bypass_cn",
  PROXY_GFW_RULE: "proxy_gfw",
  CHECK_HUB_ADDRESS: "check_hub_address",
  SEARCH_CONNECTION_TIP: "search_connection_tip",
  EDIT_INTERFACE_TITLE: "edit_interface_title",
  EDIT_TESTING_URL: "edit_testing_url",
  EDIT_LOCAL_HTTP_SERVER_TITLE: "edit_local_http_server_title",
  APP_HOME_PAGE: "app_home_page",
  APP_DOCS: "app_docs",
  APP_REPORT_ISSUE: "app_report_issue",
  HIJACK_DNS_SWITCH_LABEL: "hijack_dns_switch_label",
  HIJACK_DNS_SWITCH_TOOLTIP: "hijack_dns_switch_tooltip",
  DISABLE_DNS_CACHE_SWITCH_LABEL: "disable_dns_cache_switch_label",
  DISABLE_DNS_CACHE_SWITCH_TOOLTIP: "disable_dns_cache_switch_tooltip",
  HIJACK_DNS_SERVICE_LABEL: "hijack_dns_service_label",
  HIJACK_DNS_SERVICE_DESC: "hijack_dns_service_desc",
  EDIT_HIJACK_DNS_SERVICE_TITLE: "edit_hijack_dns_service_title",
  BLOCK_QUIC_SWITCH_LABEL: "block_quic_switch_label",
  BLOCK_QUIC_SWITCH_TOOLTIP: "block_quic_switch_tooltip",
  SENSITIVE_INFO_MODE_SWITCH_LABEL: "sensitive_info_mode_switch_label",
  SENSITIVE_INFO_MODE_SWITCH_TOOLTIP: "sensitive_info_mode_switch_tooltip",
  SHOULD_FIND_PROCESS_SWITCH_LABEL: "should_find_process_switch_label",
  SHOULD_FIND_PROCESS_SWITCH_TOOLTIP: "should_find_process_switch_tooltip",
  AUTO_LAUNCH_SWITCH_LABEL: "auto_launch_switch_label",
  AUTO_LAUNCH_SWITCH_TOOLTIP: "auto_launch_switch_tooltip",
  AUTO_CONNECT_SWITCH_LABEL: "auto_connect_switch_label",
  AUTO_CONNECT_SWITCH_TOOLTIP: "auto_connect_switch_tooltip",
  COPY: "common.copy",
  UPDATE_SUBSCRIPTION_PROXIES: "update_subscription_proxies",
  LOCAL_SERVERS: "local_servers",
  RESET_CONFIG: "reset_config",
  RESET_CONFIG_TIPS: "reset_config_tips",
  LANGUAGE: "common.language",
  THEME: "theme",
  DARK: "dark",
  LIGHT: "light",
  EN_US: "en_us",
  ZH_CN: "zh_cn",
  SYSTEM: "common.system",
  TOTAL: "common.total",
  SPEED: "common.speed",
  DOMAIN: "common.domain",
  MODE_TOOLTIP: "common.mode_tooltip",
  EDIT_MODE_TOOLTIP: "common.edit_mode_tooltip",
  PROXY_MODE_TOOLTIP: "common.proxy_mode_tooltip",
  STATISTICS: "common.statistics",
  PAYLOAD: "common.payload",
  POLICY: "common.policy",
  SEARCH_RULE_TIP: "search_rule_tip",
  DISMISS: "common.dismiss",
  CUSTOMIZED: "customized",
  ADD_RULE: "common.add_rule",
  EMPTY_DNS: "notification.empty_dns",
  ADD: "common.add",
  DNS_OPTION_TIP: "dns_option_tip",
  ADD_DNS_OPTION: "common.add_dns_option",
  DNS_OPTION_TITLE: "common.dns_option_title",
  DNS_OPTION_DESC: "common.dns_option_desc",
  INVALID_DNS_PREFIX: "notification.invalid_dns_prefix",
  CHECK_UPDATE_ERROR: "notification.check_update_error",
  NO_NEW_VERSION: "notification.no_new_version",
  DNS_SERVER_NUM_MSG: "dns_server_num_msg",
  NEW_CUSTOMIZED_RULE: "new_customized_rule",
  NEW_SHADOWSOCKS: "new_shadowsocks",
  NEW_ANYTLS: "new_anyTLS",
  NEW_HTTP: "new_http",
  NEW_SOCKS5: "new_socks5",
  NEW_CUSTOMIZED_DNS_OPTION: "new_customized_dns_option",
  NEW_IMPORT_SUBSCRIPTION_URL: "new_import_subscription_url",
  IMPORT_PROXY_URI: "import_proxy_uri",
  IMPORT_PROXY_FROM_CLASH_YAML: "import_proxy_from_clash_yaml",
  TOKEN: "token",
  TUN: "tun",
  SYSTEM_SETTING: "system_setting",
  HIJACK_DNS_ALWAYS_RESET_SWITCH_LABEL: "hijack_dns_always_reset_switch_label",
  HIJACK_DNS_ALWAYS_RESET_SWITCH_TOOLTIP:
    "hijack_dns_always_reset_switch_tooltip",
  SUCCESS_DNS: "success_dns",
  FAIL_DNS: "fail_dns",
  EDIT_SHADOWSOCKS: "edit_shadowsocks",
  EDIT_ANYTLS: "edit_anyTLS",
  EDIT_HTTP: "edit_http",
  EDIT_SOCKS5: "edit_socks5",
  EDIT_CUSTOMIZED_RULE: "edit_customized_rule",
  CONFIG_FILE_DIR_MODAL_TITLE: "config_file_dir_modal_title",
  DIAGNOSTICS: "diagnostics",
  REQUIRED: "required",
  MIXED_MODE: "mixed_mode",
  FAKE_IP_SWITCH_LABEL: "fake_ip_switch_label",
  FAKE_IP_SWITCH_TOOLTIP: "fake_ip_switch_tooltip",
  RESTART_APP: "restart_app",
  RESTART_APP_ALERT: "restart_app_alert",
  EXIT_APP: "exit_app",
  GENERAL: "common.general",
  ADVANCED: "common.advanced",
  DEFAULT: "common.default",
  REMARK: "remark",
  EDIT_SUBSCRIPTION: "edit_subscription",
  URL: "url",
  INVALID_DNS_FOR_HTTP_PROXY: "invalid_dns_for_http_proxy",
};
