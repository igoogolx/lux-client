...
import EditDnsItem from "@/components/pages/Setting/Dns/EditDnsItem";
import { DnsTypeEnum } from "@/components/Core/EditItemWithSelectDialog";
  ...
  return (
    <Card className={styles.card}>
      <EditDnsItem
        ...
        t(TRANSLATION_KEY.REMOTE_DNS_DESC)
      } />

      <EditDnsItem
        type={setting.dns.remote.type as DnsTypeEnum}
        items={localDnsOptions.current} value={setting.dns.local.value} onSubmit={(data) => {
        onSubmit({
          ...setting.dns,
          local: {
            ...setting.dns.local,
        ...
        t(TRANSLATION_KEY.LOCAL_DNS_DESC)
      } />
      {hasDoh && (
        <EditDnsItem
          type={setting.dns.boost.type as DnsTypeEnum}
          items={boostDnsOptions.current} value={setting.dns.boost.value} onSubmit={(data) => {
          onSubmit({
            ...setting.dns,
            boost: {
              ...setting.dns.boost,
          ...
          title={
            t(TRANSLATION_KEY.BOOST_DNS_LABEL)
          } desc={
          t(TRANSLATION_KEY.BOOST_DNS_DESC)
        } />)}
    </Card>
  );
}
